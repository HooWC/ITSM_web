...
function DeleteItem() {
    ...
    // Send delete request
    $.ajax({
        url: window.AppRoot + 'Ajax/DeleteDepartment',
        method: 'POST',
...
function searchDepartments() {
    ...
    $.ajax({
        url: window.AppRoot + 'Ajax/SearchDepartment',
        method: 'GET',
        data: {
...
function updateDepartmentTable(data) {
    ...
    if (data.length === 0) {
        tableBody.append('<tr><td colspan="6" class="text-center">No matching Department found</td></tr>');
        IncidentItems = 0;
        IncidentPages = 0;
        updatePaginationInfo();
        updatePaginationButtons();
    ...
    $.each(data, function (index, dep) {
        var row = `
                    <tr class="incident-item" data-id="${dep.id}">
                        <td><input type="checkbox" class="item-checkbox"></td>
                        <td class="inc-tab-incident-number" data-label="Name">
                            <a href="/Department/Department_Info?id=${dep.id}">${dep.name}</a>
                        </td>
