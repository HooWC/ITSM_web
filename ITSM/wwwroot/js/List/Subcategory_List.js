...
function DeleteItem() {
    ...
    $.ajax({
        url: window.AppRoot + 'Ajax/DeleteSubcategory',
        method: 'POST',
...
function searchCategorys() {
    ...
    $.ajax({
        url: window.AppRoot + 'Ajax/SearchSubcategory',
        method: 'GET',
        data: {
...
function updateCategoryTable(data) {
    ...
    if (data.length === 0) {
        tableBody.append('<tr><td colspan="7" class="text-center">No matching Sucategory found</td></tr>');
        IncidentItems = 0;
        IncidentPages = 0;
        updatePaginationInfo();
        updatePaginationButtons();
    ...
    $.each(data, function (index, sucategory) {
        var row = `
                    <tr class="incident-item" data-id="${sucategory.id}">
